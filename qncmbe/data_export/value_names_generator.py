'''
This file defines all the data signals that can be collected by data_export,
and stores the relevant information so that data_export knows how to get them.

The main result is the OrderedDict object called value_names_database.

Each key of value_names_database is the value name. E.g., 'Ga1 tip measured'
The corresponding value is a dictionary (i.e., value_names_database is a dictionary of dictionaries)
This subdictionary must contain the following keys:
  - "Location": essentially where the data is stored (so data_export knows how to find and read it)
  - "Local value name": the "proper" name of the data value. So, e.g., "Ga1 tip measured" is actually stored on the Molly computer as "Instances.Ga1_tip.Measured"
  - "Measurement type": is it temperature, power, etc.
  - "Units": units used. Or, if it's a binary signal, specifying what 0 means.

This info is used by data_export. So, e.g., since
value_names_database['Ga1 tip measured']['Location'] == 'Molly'
the data_export utility knows to look on the Molly computer for this data signal.
'''
...
def generate_value_names_csv(fname = "value_names_database.csv"):
